...
| #10 | 2026-08-28 | v3.1 | 架构补全 + compaction 防护：适配器注册表（D24：统一接口 + custom 通道，加新 agent 零改核心）；compaction 防护（D25：ZCode 压缩物理删消息实测确认，摘要捕获 + 丢数据警告双层防护）；服务安装改注册表 Run 键（免管理员）；README 补 MCP 接入章节 + 守护必要性；Adapter SDK 文档（`docs/adapters/README.md`） |
...
| **D24** | **适配器注册表：统一 SessionSourceAdapter 接口 + `.sessionrelay/adapters/*.js` custom 通道**（Review #10） | 加新 agent = 零改核心代码（丢一个 .js 文件即可），配得上"不属于任何厂商"的定位；if/else 硬编码不配 | 继续硬编码（每加一个 agent 改三处核心）；只支持编译期插件（门槛高） |
| **D25** | **compaction 防护双管齐下：捕获压缩摘要（system 角色）+ 检测丢数据并警告**（Review #10） | ZCode 压缩物理删消息是实测确认的事实；守护缩短窗口至 30 秒 + 摘要兜底 + 用户知情，三层防线 | 只依赖守护（窗口内仍可能丢）；不警告（用户不知情） |

...
13. [优缺点分析](https://www.qianwen.com/chat/b5403abd0e7e48829e8a078f0f5b3322#十三优缺点分析)
14. [风险与应对](https://www.qianwen.com/chat/b5403abd0e7e48829e8a078f0f5b3322#十四风险与应对)
15. [开发计划](https://www.qianwen.com/chat/b5403abd0e7e48829e8a078f0f5b3322#十五开发计划)
16. [商业化路径](https://www.qianwen.com/chat/b5403abd0e7e48829e8a078f0f5b3322#十六商业化路径)
17. [总结](https://www.qianwen.com/chat/b5403abd0e7e48829e8a078f0f5b3322#十七总结)
...
> **SessionGraph 是一个项目级跨 Agent 会话记忆工具，让 AI 编程助手能够跨会话、跨工具回忆历史讨论内容。**
...
| 2    | **本地优先**          | 数据存本地，零外部依赖（不需要云服务、向量数据库） |
| 3    | **类 CodeGraph 范式** | 预索引 → 结构化存储 → 按需查询，不做运行时遍历     |
| 4    | **CLI 交互**          | 独立命令行工具，不依赖 AI 环境也能使用             |
| 5    | **增量同步**          | 不全量重建，只处理新增/变更的会话，支持 `--watch`  |
| 6    | **MCP Server**        | AI 通过 MCP 协议按需查询，单次调用精准返回         |
| 7    | **项目级隔离**        | 每个项目独立的记忆空间                             |
| 8    | **跨 Agent 共享**     | 不同 agent（Claude/Zcode/DSH 等）共享同一份记忆    |
...
> **项目级跨 Agent 会话记忆层。**
>
> 不管你用 Claude、Zcode、DSH 还是任何 AI 工具，所有讨论记忆统一存储、统一检索、跨 agent 共享。
...
interface UnifiedSession {
  ...
  source_session_id: string;   // 原始 agent 的会话ID
  project_id: string;
  created_at: string;
...
| 大厂/官方内置    | 第三方工具失去价值 | 速度 + 本地化/隐私叙事 + 跨 agent 差异化 |
...
| 任务                      | 交付物                 |
| ------------------------- | ---------------------- |
| MCP Server（stdio）       | AI 可调用的 6 个 Tools |
...
> **SessionGraph = 本地会话仓库 + 自动标签 + 全文搜索 + MCP 接口 + 增量同步 + 跨 Agent 共享。**
>
> 不是图谱，是**带结构化标签的跨 Agent 会话搜索引擎**。先跑通价值，再按需叠加复杂度。
...
| 差异化         | ⭐⭐⭐⭐     | "跨 agent + 本地"目前无人做               |
| 技术可行性     | ⭐⭐⭐⭐⭐    | SQLite + FTS5 + MCP，无黑科技             |
| 竞争压力       | ⭐⭐⭐      | 大厂和 claude-mem 是潜在威胁              |
| 用户获取难度   | ⭐⭐⭐      | 需要改变"主动存储"的习惯                  |
| 商业化空间     | ⭐⭐⭐      | 开源工具变现不易，但有路径                |
| 时机           | ⭐⭐⭐⭐     | 赛道刚起步，但窗口期 6-12 月              |
| **综合**       | **⭐⭐⭐⭐** | **值得做，但要快**                        |
...
4. **传播**。写一篇"我在 5 个 AI 工具之间共享记忆"的博文
5. **观察**。密切关注 claude-mem 是否扩展、大厂是否出开发者版本
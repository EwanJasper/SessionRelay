...
### G2 不变量演进（related 落地时登记）

G2 的“恒 15 工具”已升级为更强断言：16 工具精确名单 + 删除类名称黑名单扫描（delete/remove/forget/purge）。防删除泄漏的意图不变，计数随只读工具增长合法演进。

### 手动项执行结果（2026-09-07 回填，§J 步骤 2 清账）

| 项 | 结果 | 明细 |
|---|---|---|
| E5 双 CLI 竞态 | ✅ 通过 | 真实双进程（dist CLI）：A 预览→B 抢先 --yes 删除成功→A 持过期快照 --yes 得 not-found；终态 0 部分删除残留、forget_log 仅 1 条（B 的） |
| H2 墓碑压测 | ✅ 通过 | 5000 行墓碑，sync 周期均耗 2.00ms（含 loadTombstones 整表载入），远低于 50ms 基线——无 per-session 点查劣化 |

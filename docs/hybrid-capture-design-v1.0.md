# 双通道捕获架构 — 混合方案 v1.0（三轮评审后否决）

> **日期**：2026-08-29
> **状态**：三轮评审完毕，用户确认后**否决**（2026-08-29）
>
> **否决理由**：收益/成本比不划算——hook 开发 3-5 天 + 永久维护负担，只换来事件级线索（无 AI 回复）。
> 用户缺 agent 支持时有三条更好的路：等 adapter（完整数据）、配置 MCP + save_note（今天就能用）、HOP 交接包。
> 更深层：去模仿 Memorix 的 hooks 是在自己弱项上跟别人强项竞争，同时稀释被动读取这个核心竞争力。
> 本方案保留作为技术调研档案，不做实施。
> **评审轮次**：①产品 ②技术架构 ③软件设计
